...
class TestArrayFunctions:
    ...
    def test_function_with_tuple_arguments(self):
        ...
        assert evaluate('any_of(tuple_objects, `value > 3`)', context) == True
        assert evaluate('all_of(tuple_objects, `value > 0`)', context) == True
        assert evaluate('none_of(tuple_objects, `value > 10`)', context) == True

    def test_complex_conditions(self):
        """Test functions with complex nested conditions"""
        context = {
            'orders': [
                {'customer': {'type': 'premium'}, 'items': ['laptop', 'mouse'], 'total': 1500},
                {'customer': {'type': 'regular'}, 'items': ['book'], 'total': 20},
                {'customer': {'type': 'premium'}, 'items': ['phone', 'case'], 'total': 800}
            ]
        }
        ...
        assert evaluate('any_of(orders, `"laptop" in items`)', context) == True
        assert evaluate('none_of(orders, `"tablet" in items`)', context) == True
        
        # Complex boolean conditions
        assert evaluate('count_of(orders, `customer.type == "premium" and total > 1000`)', context) == 1

    def test_grammar_edge_cases(self):
        """Test grammar parsing edge cases"""
        context = {
            'data': [1, 2, 3],
            'users': [{'name': 'test', 'score': 5}]
        }
        ...
        assert evaluate('count_of(data) == 3', context) == True
        assert evaluate('count_of(data) > 2', context) == True
        assert evaluate('count_of(data) <= 5', context) == True
        
        # Function calls in logical expressions
        assert evaluate('count_of(data) > 0 and any_of(users, `score > 0`)', context) == True
        assert evaluate('count_of(data) == 0 or all_of(users, `score > 0`)', context) == True
    ...
    def test_parentheses_and_precedence(self):
        ...
        assert evaluate('(count_of(numbers)) == 5', context) == True
    ...
    def test_function_name_collision_prevention(self):
        ...
        assert evaluate('count_of_items == 10', context) == True
        assert evaluate('any_of_these', context) == True
    ...
    def test_function_with_variable_paths(self):
        ...
        assert evaluate('any_of(project.teams, `"Backend" in name`)', context) == True
    ...
    def test_functions_with_float_values(self):
        ...
        assert evaluate('any_of(prices, `price == 25.99`)', context) == True
        assert evaluate('all_of(prices, `price >= 5.0`)', context) == True
    ...
    def test_functions_with_boolean_logic(self):
        ...
        assert evaluate('count_of(tasks, `priority == "high" and completed`)', context) == 1
        assert evaluate('count_of(tasks, `priority == "high" and completed == false`)', context) == 1
        
        # Complex OR conditions  
        assert evaluate('any_of(tasks, `completed or days > 8`)', context) == True
        assert evaluate('all_of(tasks, `priority == "high" or days > 0`)', context) == True
    ...
    def test_function_precedence_with_operators(self):
        ...
        assert evaluate('count_of(items) > 2 and count_of(items) < 10', context) == True
    ...
    def test_functions_with_special_characters(self):
        ...
        assert evaluate('any_of(messages, `"$" in text`)', context) == True
        assert evaluate('count_of(messages, `"," in text`)', context) == 1
